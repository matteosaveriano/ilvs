...
%GPR(1).logtheta = [log(1); log(sqrt(1.0)); log(sqrt(0.1))];
...
gpIn  = [gpIn demos{demoNum(1)}.pos(:,sInd)];
gpOut = [gpOut demos{demoNum(1)}.vel(:,sInd)];
gpIn  = [gpIn demos{demoNum(2)}.pos(:,sInd)];
gpOut = [gpOut demos{demoNum(2)}.vel(:,sInd)];
...
GPR_reshape = trainGPIncremental(GPR_reshape, gpResX, trainingData, 1, false);
timeIncrGp = toc;
...
if(plotResult)
    ...
    h(2) = plot(sp,GPR_reshape(1).data.in(1,:),GPR_reshape(1).data.in(2,:),'o','color',[183, 65, 14]./255,'LineWidth',2,'Markersize',10);
    %h(2) = plot(sp,GPR_reshape(1).data.in(1,16:end),GPR_reshape(1).data.in(2,16:end),'o','color',[200/255, 0, 0],'LineWidth',2,'Markersize',10);
%     h(2) = plot(sp,GPR_reshape(1).data.in(1,1:17),GPR_reshape(1).data.in(2,1:17),'o','color',[183, 65, 14]./255,'LineWidth',2,'Markersize',10);
...
for demoIt=demoNum         
    ...
    % Save errors
    allBestDist(distIt)  = velocityRMSE_learned_par_fast(fn_handle, GPR_reshape, pDemo, vDemo, truncValues);
    distIt = distIt + 1;
end
